...
use egui::{
    Color32, CornerRadius, Image, Painter, Pos2, Rect, RichText, Sense, Stroke, StrokeKind, Ui,
    Vec2, include_image, pos2, vec2,
};
...
impl DS4UApp {
    pub(crate) fn render_inputs_section(&self, ui: &mut Ui) {
        ...
        let stick_colors = [c.widget_inactive(), c.accent(), c.text()];

        let avail_w = ui.available_width();
        let avail_h = ui.available_height();
        let side = avail_w.min(avail_h).clamp(320.0, 760.0);

        ui.vertical_centered(|ui| {
            let (response, painter) = ui.allocate_painter(vec2(side, side), Sense::hover());
            ...
            Self::render_button(
                &painter,
                map(SVG_CREATE_BTN.0, SVG_CREATE_BTN.1),
                meta_r,
                buttons & BTN_CREATE != 0,
                c.accent(),
                c.text(),
            );
            Self::render_button(
                &painter,
                map(SVG_OPTIONS_BTN.0, SVG_OPTIONS_BTN.1),
                meta_r,
                buttons & BTN_OPTIONS != 0,
                c.accent(),
                c.text(),
            );
            ...
            Self::render_button(
                &painter,
                map(SVG_SQUARE.0, SVG_SQUARE.1),
                fb_r,
                buttons & BTN_SQUARE != 0,
                c.accent(),
                c.text(),
            );
            Self::render_button(
                &painter,
                map(SVG_CROSS.0, SVG_CROSS.1),
                fb_r,
                buttons & BTN_CROSS != 0,
                c.accent(),
                c.text(),
            ...
            Self::render_button(
                &painter,
                map(SVG_CIRCLE.0, SVG_CIRCLE.1),
                fb_r,
                buttons & BTN_CIRCLE != 0,
                c.accent(),
                c.text(),
            );
            Self::render_button(
                &painter,
                map(SVG_TRIANGLE.0, SVG_TRIANGLE.1),
                fb_r,
                buttons & BTN_TRIANGLE != 0,
                c.accent(),
                c.text(),
            );
    ...
    fn render_dpad_button(
        ...
        let s = PathShape {
            points,
            closed: true,
            fill: if dpad.contains(&dpad_active) {
                accent_color
            } else {
                Color32::TRANSPARENT
            },
    ...
    fn render_live_stick(
        ...
        p.circle_stroke(center, radius, egui::Stroke::new(1.0, Color32::WHITE));
        ...
        let dot_r = (radius * 0.28).clamp(4.0, 10.0);
        let inset = dot_r + 2.0;
        let dot = pos2(
            center.x + nx * (radius - inset),
            center.y + ny * (radius - inset),
        );
        p.circle_filled(dot, dot_r, colors[0]);
        p.circle_stroke(dot, dot_r, egui::Stroke::new(1.0, colors[2]));
    }
}
