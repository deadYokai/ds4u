...
impl DS4UApp {
    pub(crate) fn render_audio_settings(&mut self, ui: &mut Ui) {
        ...
        if ui
            .checkbox(&mut self.microphone.enabled, "Microphone Enabled")
            .changed()
        {
        ...
        ui.horizontal(|ui| {
            if ui
                .selectable_value(&mut self.microphone.led_state, MicLedState::Off, "Off")
                .clicked()
            {
                self.apply_microphone();
            }
            if ui
                .selectable_value(&mut self.microphone.led_state, MicLedState::On, "On")
                .clicked()
            {
                self.apply_microphone();
            }
            if ui
                .selectable_value(&mut self.microphone.led_state, MicLedState::Pulse, "Pulse")
                .clicked()
            {
                self.apply_microphone();
            }
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .selectable_label(self.audio.speaker_mode == SpeakerMode::Both, "Both")
                .clicked()
            {
        ...
        ui.horizontal(|ui| {
            ui.label("Level:");
            if ui
                .add(Slider::new(&mut self.audio.volume, 0..=255).text(""))
                .changed()
            {
